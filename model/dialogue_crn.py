...
class CognitionNetwork(nn.Module):
    def __init__(self, n_features=200, dropout=0.2, cuda_flag=False, reason_steps=None):
        """
        ...
        # self.smax_fc = nn.Linear(n_features * 4)
...
class DialogueCRN(nn.Module):
    def __init__(self, base_model='LSTM', base_layer=2, input_size=None, hidden_size=None, n_speakers=2,
                 dropout=0.2, cuda_flag=False, reason_steps=None):
        """
        ...
        self.cognition_net = CognitionNetwork(n_features=2 * hidden_size, dropout=dropout, cuda_flag=cuda_flag, reason_steps=reason_steps)
        print(self)

    def forward(self, U, qmask, seq_lengths):
        # U: max_seq_length x batch_size x hidden_size
        # qmask: max_seq_length x batch_size x num_speakers
        ...
        hidden_states = self.cognition_net(U_s, U_p, seq_lengths)
