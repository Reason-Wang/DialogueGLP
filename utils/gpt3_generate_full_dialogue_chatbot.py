...
def get_input_from_DD_dialogue(df):
    prompt = "Marv is a chatbot that is easily emotional.\n\n"
    length = len(df)
    if length <= 1:
...
def get_input_from_meld_dialogue(df):
    prompt = "The following is a conversation. Fill in the conversation with one utterance.\n\n"
    length = len(df)
    if length <= 1:
        print("Skipped dialogue with 1 utterance")
        return None
    for i, row in df.iterrows():
        ...
        if i == length - 2:
            break
    prompt = prompt + df.loc[length-1, 'Speaker'] + ":"
    return prompt
...
@click.command()
@click.option('--task', type=str, default=None, required=True)
@click.option('--input_path', type=str, default=None, required=True)
@click.option('--output_path', type=str, default=None, required=True)
